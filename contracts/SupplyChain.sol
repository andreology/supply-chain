...
contract suppplyChain {
    ...
    mapping(uint => ownership) public ownerships; // ownerships by ownership ID (owner_id)
    mapping(uint32 => uint32[]) public productTrack; //ownerships by Proudct ID (product_id) / movement track for a product

    function addParticipant(string memory _name, string memory _pass, address _pAdd, string memory _pType) public returns (uint32) {
        uint32 userId = particpant_id++;
        participants[userId].userName = _name;
        participantsp[userId].password = _pass;
    ...
    function addProduct(uint32 _ownerId, string memory _modelNumber, string memory _partNumber, string memory _serialNumber, uint32 _productCost) public returns (uint32) {
        if(keccak256(abi.encodePacked(participants[_owner_id].participantType)) == kekkak256("Manufacturer")) {
    ...
    function getProduct(uint _productId) public view returns(string memory, string memory, string memory, uint32, address, uint32) {
    ...
    function newOwner(uint32 _userId, uint32 _user2Id, uint32 _prodId) onlyOwner(_prodId) public returns(bool) {
        ...
        if(keccak256(abi.encodePacked(p1.participantType)) == keccak256("Manufacturer") 
         && keccak256(abi.encodePacked(p2.participantType)) == keccak256("Supplier")) {
             ...
             ownerships[ownership_id].ownerId = _userId;
    ...
    function Provenance(uint32 _prodId) external view returns (uint32[] momory) {
